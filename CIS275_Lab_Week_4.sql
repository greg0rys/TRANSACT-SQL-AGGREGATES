...
SELECT CONVERT(char(15), title_genre.genre) AS [Genre],
	   COUNT(title_basics.primaryTitle) AS [Count]
FROM title_genre 
JOIN title_basics 
	ON title_genre.tconst = title_basics.tconst
GROUP BY title_genre.genre
ORDER BY Count DESC;
...
SELECT title_episode.seasonNumber AS [Season],
	   COUNT(title_basics.originalTitle) AS [Number of Episodes], 
	   SUM(title_ratings.numVotes) AS [Total Votes by Season]
FROM title_basics 
JOIN title_episode 
	ON title_episode.tconst = title_basics.tconst
JOIN title_basics AS TB2 
	ON title_episode.parentTconst = TB2.tconst
LEFT JOIN title_ratings  
	ON title_episode.tconst = title_ratings.tconst
WHERE TB2.primaryTitle = 'M*A*S*H'
GROUP BY title_episode.seasonNumber
ORDER BY title_episode.seasonNumber;
...
SELECT CONVERT(char(15), title_genre.genre) AS [Genre], 
	   ...
	   STR(CAST(100 * COUNT(title_basics.primaryTitle) AS DECIMAL) / SUM(COUNT(*)) OVER(),6,2) + '%' AS [Percent]
FROM title_genre 
JOIN title_basics 
	ON title_genre.tconst = title_basics.tconst
GROUP BY title_genre.genre
...
SELECT TOP 10 
year_gender_totals.Gender AS [Gender],
 CAST(names.Metaphone AS char(10)),
 COUNT(names.Name) AS [Total Names],
 SUM(name_counts.NameCount) AS [Total Babies]
FROM year_gender_totals
JOIN name_counts ON year_gender_totals.YearGenderTotalID = name_counts.FK_YearGenderTotalID
JOIN names ON name_counts.FK_NameID = names.NameID
GROUP BY year_gender_totals.Gender,names.Metaphone
ORDER BY [Total Babies] DESC;
...
WHERE names.Metaphone = 'JN'
	AND year_gender_totals.Gender = 'M'
GROUP BY names.Name
ORDER BY [Total Babies] DESC;
...
SELECT names.Metaphone AS [Metaphone], 
	   year_gender_totals.Year AS [Year], 
	   SUM(name_counts.NameCount) AS [Total Babies]
FROM year_gender_totals
	JOIN name_counts ON year_gender_totals.YearGenderTotalID = name_counts.FK_YearGenderTotalID
	JOIN names on name_counts.FK_NameID = names.NameID
WHERE names.Metaphone = 'JN'
	AND year_gender_totals.Gender = 'M'
GROUP BY names.Metaphone, year_gender_totals.Year
ORDER BY  [Year] ASC, [Total Babies] DESC;
...
SELECT CAST(names.Name AS char(10)) AS [Name], 
	   year_gender_totals.Year AS [Year], 
	   SUM(name_counts.NameCount) AS [Total Babies]
FROM year_gender_totals
	JOIN name_counts ON year_gender_totals.YearGenderTotalID = name_counts.FK_YearGenderTotalID
	JOIN names ON name_counts.FK_NameID = names.NameID
WHERE names.Name = 'Corey' -- check each row to make sure the name is Corey 
GROUP BY [Year], [Name]
HAVING SUM(name_counts.NameCount) >= 100; -- make sure we only include sets that have a sum greater than or equal too 100
...
SELECT CAST(SHOW.Genre AS char(20)) AS [Genre], 
	   SUM(DATEDIFF(mi, SCHEDULE.StartTime, SCHEDULE.EndTime)) AS [Total Minutes], 
	   STR(SUM(DATEDIFF(mi, SCHEDULE.StartTime, SCHEDULE.EndTime)) * 100.0 / SUM(SUM(DATEDIFF(mi, SCHEDULE.StartTime, SCHEDULE.EndTime))) OVER () ,6,2) + '%' AS [Percentage]
FROM SHOW
...
USE TV
SELECT CAST(SHOW.Title AS char(20)) AS [Title], 
	   SUM(DATEDIFF(mi, SCHEDULE.StartTime, SCHEDULE.EndTime)) AS [Total Minutes],
	   STR(SUM(DATEDIFF(mi, SCHEDULE.StartTime, SCHEDULE.EndTime)) * 100.0 / SUM(SUM(DATEDIFF(mi, SCHEDULE.StartTime, SCHEDULE.EndTime))) OVER () ,6,2) + '%' AS [Percentage]
FROM SHOW
	JOIN SCHEDULE ON SHOW.ShowID = SCHEDULE.FK_ShowID
GROUP BY SHOW.SeriesNum, [Title]
...
SELECT COUNT(DISTINCT SHOW.ShowID)AS [Distinct Shows], 
	   COUNT(SHOW.Title) AS [Total Shows],  
	   COUNT(SHOW.Title) - COUNT(DISTINCT SHOW.ShowID) AS [Repeats], 
	   STR(CAST(100 *  (COUNT(SHOW.Title) - COUNT(DISTINCT SHOW.ShowID)) AS DECIMAL) / SUM(COUNT(SHOW.Title)) OVER(),6,2) + '%' AS [Percent]
FROM SHOW
	JOIN SCHEDULE ON SHOW.ShowID = SCHEDULE.FK_ShowID;
